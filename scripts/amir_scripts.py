...
class MainWindow(GObject.GObject):
    ...
    def __init__(self):
        ...
        try:
            ...
            icon22 = GdkPixbuf.Pixbuf.new_from_file(
                os.path.join(icondir, "22.png"))
            icon32 = GdkPixbuf.Pixbuf.new_from_file(
                os.path.join(icondir, "32.png"))
            icon48 = GdkPixbuf.Pixbuf.new_from_file(
                os.path.join(icondir, "48.png"))
            icon64 = GdkPixbuf.Pixbuf.new_from_file(
                os.path.join(icondir, "64.png"))
            icon128 = GdkPixbuf.Pixbuf.new_from_file(
                os.path.join(icondir, "128.png"))
            mainWin.set_default_icon_list(
                [icon16, icon22, icon32, icon48, icon64, icon128])
    ...
    def ledgerNotebookReport(self, sender):
        reportwin = notebookreport.NotebookReport(
            notebookreport.NotebookReport.LEDGER)

    def subledgerNotebookReport(self, sender):
        reportwin = notebookreport.NotebookReport(
            notebookreport.NotebookReport.SUBLEDGER)
    ...
    def doBackup(self, sender):
        ...
        handle_database.backup(
            location, self.builder.get_object("backupName").get_text())
    ...
    def quitMainWindow(self, sender):
        msgbox = Gtk.MessageDialog(self.window, Gtk.DialogFlags.MODAL, Gtk.MessageType.WARNING,
                                   Gtk.ButtonsType.OK_CANCEL, _("Are you sure to close the app?"))
    ...
    def loggedIn(self):
        ...
        chqDueDate = config.db.session.query(Cheque.chqDueDate).filter(
            or_(Cheque.chqStatus == 1, Cheque.chqStatus == 3)).filter(Cheque.chqDelete != 1)
    ...
    def loginProcess(self, sender=0):
        ...
        if config.db.session.query(Users).count() == 0:
            ...
            config.db.session.add(user)
            config.db.session.commit()
        self.loginUser = config.db.session.query(Users).filter(
            Users.username == str(username.get_text())).first()
        if self.loginUser:
            permission = config.db.session.query(Permissions).filter(
                Permissions.id == self.loginUser.id).first()
            # self.permissionValue = int(permission.value)
            utility.setPermissionValue(int(permission.value))
        ...
        if active_index != config.currentdb - 1:
            ...
            if result == Gtk.ResponseType.OK:
                ...
                config.db = database.Database(
                    config.dblist[active_index], config.db_repository, config.echodbresult)
    ...
    def localeChanged(self, sender, new_locale):
        ...
        self.msgbox = Gtk.MessageDialog(
            self.window, Gtk.DialogFlags.MODAL, Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, msg)
    ...
    def alert_dialog(self, text):
        ...
        if alertBarCnt:
            yPos = float(
                self.infobar[0].get_size_request().height) * float(alertBarCnt)
        infobar = Gtk.InfoBar()
        infobar.get_content_area().add(Gtk.Label(label=text))
    ...
    def on_win_resize(self, sender, a):
        ...
        if len(self.infobar):
            for ib in self.infobar:
                ib.set_size_request(
                    self.background.get_allocated_width(), self.h)
...
if __name__ == "__main__":
    ...
    GObject.signal_new("database-changed", MainWindow,
