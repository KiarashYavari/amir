...
class Product(productgroup.ProductGroup):
        ...
        for g, p in result:
            ...
            if g.id != last_gid:
                grouprow = self.treestore.append(None, (utility.readNumber(
                    g.code), utility.readNumber(g.name), "", "", "", "", 0))
                last_gid = g.id
        ...
        if iter != None:
            if self.treestore.iter_parent(iter) == None:
                ...
                self.builder.get_object(
                    "proLocEntry").set_text(product.location)
                self.builder.get_object(
                    "proDescEntry").set_text(product.productDesc)
                self.builder.get_object("discFormulaEntry").set_text(
                ...
                self.builder.get_object(
                    "oversell").set_active(product.oversell)
                self.builder.get_object("uMeasureEntry").set_text(
    ...
    def saveRow(self, treeiter, data):
        if len(data) == 4:
            ...
            self.treestore.set(
                treeiter, 0, data[0], 1, data[1], 2, data[2], 3, data[3], 4, data[4])
